...
PARAM_GRIDS = {
    ...
    "XGBoost": {
        "model": xgb.XGBClassifier(eval_metric="logloss", n_jobs=4, tree_method='gpu_hist') if XGB_AVAILABLE else None,
        "params": {
            "n_estimators": [100, 200],
...
def run_experiment(args):
    ...
    for cancer in cancers:
        ...
        print(f"   样本数: {len(y)} | 特征维数: {X.shape[1]} (RNA 维度: {n_rna})")
        ...
        for seed in range(args.n_seeds):
            ...
            X_train, X_test = X[train_idx].copy(), X[test_idx].copy() # 使用 copy 防止切片赋值警告
    ...
    out_file = "results/baseline_summary.json"
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)
    
    # 打印最终表格汇总
    print("\n" + "="*80)
    print("🏆 Baseline 结果汇总 (Mean AUC ± SD)")
